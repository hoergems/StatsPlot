...
class PlotStats:
    def __init__(self, dir, save_plots, show_particles, plot_emds, collision_is_failed):        
        ...
        print "collision is failed: " + str(collision_is_failed)
        reward_model = {}
	...
	self.plot_num_steps("steps_stats",
		            dir=dir,
		            finish_when_collided=True)
	self.plot_mon_stats("mon_stats",
		            dir=dir,
		            finish_when_collided=True)
	'''self.plot_stat_from_txt_file("mon_stats",
			             "mon",
			             dir=dir,
			             y_label="SNM")'''
	...
	m_covs, files = self.get_files(output_file_str, dir)
		...
		with open(file, "r") as f:
			...
			if cov_str in line:                                             
			    m_cov = float(line.split(" ")[2])
			elif "MoN: " in line and not "time" in line:
			    print "MON FOUND!"
			    mon = float(line.strip().split(": ")[1])
			    mons.append(mon)
	        print mons
		n, min_max, mean, var, skew, kurt = scipy.stats.describe(np.array(mons))
		with open(os.path.join(dir, "out_" + str(output_file_str) + "_" + str(m_cov) + ".txt"), "a+") as f:
		    f.write(file_str + " " + str(m_cov) + ": \n")
		    f.write("mean per run: " + str(mean) + " \n")
		    f.write("variance: " + str(var) + " \n")
    ...
    def plot_planner_usage(self, output_file_str, dir="stats", finish_when_collided=True):
	m_covs, files = self.get_files(output_file_str, dir)
	d = dict()
	algorithm = ""
	for file in files:
    ...
    def plot_num_succesful_runs2(self, output_file_str, dir="stats", finish_when_collided=True):
	pass
			    
	            
                
    def plot_num_succesful_runs(self, output_file_str, dir="stats", finish_when_collided=True):
        m_covs, files = self.get_files(output_file_str, dir)
        d = dict()
        algorithm = ""
        for file in files: 
            ...
            with open(file, "r") as f:
                ...
                for line in f:                                      
                    ...
                    if "collided: true" in line or "Collision detected: True" in line:			
                        ...
                        num_runs += 1                       
                        block = False
                    elif "R:" in line and not "LINEAR" in line and not block and not algorithm == "nnl":			
		        rew = float(line.split(": ")[-1].strip())		        
			if (rew > 800):
			    num_succ += 1
			#print float(line.split(": ")[-1].strip())
			'''elif "R: 1000" in line and not block and not algorithm == "nnl":
			    num_succ += 1'''
		    elif "Terminal: True" in line and algorithm == "nnl" and not block:
			num_succ += 1
                    '''elif "Num successes:" in line:
                        num_succ = float(line.strip().split(": ")[1])'''
                       
            #actual_successful_runs = num_succ - num_not_succ
            actual_successful_runs = num_succ
            print "file " + str(file)
    ...
    def plot_num_steps(self, output_file_str, dir="stats", finish_when_collided=True):
	m_covs, files = self.get_files(output_file_str, dir)
        d = dict()
        algorithm = ""
        arrs = []
	    ...
	    with open(os.path.join(dir, "out_" + str(output_file_str) + "_" + str(m_cov) + ".txt"), "a+") as f:
		f.write(file_str + " " + str(m_cov) + ": \n")
		f.write("mean num steps: " + str(num_steps) + " \n")
    ...
    def plot_stat_from_txt_file(self, stat_str, output_file_str, dir="stats", y_label=""):
        ...
        for file in files:
			...
			print "mean line: " + str(line)
                        mean = float(line.split(": ")[1].strip())                        
                    elif "variance" in line and not "inc" in line:
                        ...
                        print "alg is " + str(alg)
                        print "mean per run is " + str(mean)
                        print "file " + str(file)
                        d[alg].append([cov_val, mean, conf])
                    if "inc_covariance" in line:
			if "process" in line:
			    x_label = "process error"
        ...
        print "stat_str " + str(stat_str)
        x_range=[min(m_covs), max(m_covs) + 0.5]
        if "reward" in stat_str:
	    min_m = -500.0
